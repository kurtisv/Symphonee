...
| POST | `/api/orchestrator/spawn` | Spawn an AI CLI for a one-shot task. Body: `{ cli, prompt, cwd?, timeout?, from?, visible? }` (cli: claude/gemini/codex/antigravity/copilot/grok/qwen; defaults to headless pipe mode) |
