...
(() => {
  // dashboard/public/settings/src/index.js
  ...
  state._aiToolsStatus = {};
  state._pwshStatus = {
    installed: false
  };
  var _aiInstalling = /* @__PURE__ */ new Set();
  ...
  var _repoSrcCache = /* @__PURE__ */ new Map();
  ...
  async function _fetchPluginRepos(source, query) {
    ...
    if (!query && cached && now - cached.ts < 6e4) return cached.list;
  ...
  async function refreshSmartSearchStatus() {
    ...
    try {
      const r = await fetch("/api/mind/embed-status").then((r2) => r2.json());
      const v = r.vectors || {};
      const ol = r.ollama || {};
  ...
  async function saveSettings() {
    ...
    try {
      ...
      const payload = {
        ...
        AiApiKeys: {
          OPENAI_API_KEY: _txt("settingsOpenaiKey") || void 0,
          GEMINI_API_KEY: _txt("settingsGeminiKey") || void 0,
          ANTHROPIC_API_KEY: _txt("settingsAnthropicKey") || void 0,
          XAI_API_KEY: _txt("settingsXaiKey") || void 0
        },
      ...
      if (data.ok) {
        ...
        closeSettings();
        showLoading("Loading...");
        const minWait = new Promise((r) => setTimeout(r, 4e3));
    ...
    input.onchange = async (e) => {
      ...
      try {
        ...
        if (result.ok) {
          ...
          const minWait = new Promise((r) => setTimeout(r, 3e3));
          await loadConfig(true);
          loadVelocity();
          await minWait;
          hideLoading();
